...
const RouteResultsPanel = ({ 
  ...
  const handleAnalyzeRoute = async (route, index) => {
    ...
    const prompt = `Phân tích an toàn cho lộ trình: ${route.distance.toFixed(2)} km, ${Math.round(route.duration)} phút, ${route.floodCount > 0 ? `có ${route.floodCount} vùng ngập` : 'không có ngập'}. Ngắn gọn dưới 40 từ tiếng Việt.`;
    ...
    try {
      ...
      const advice = data.candidates?.[0]?.content?.parts?.[0]?.text || "Không thể kết nối AI.";
      ...
      <div className="glass-panel results-header-compact">
        <div>
          <h3 className="results-title-compact">
            <RouteIcon size={16} className="text-indigo-600" /> {routes.length} tuyến đường
          </h3>
        </div>
            ...
            >
              ...
              <div className="route-card-header">
                <div className="route-card-header-left">
                  <div className={`route-number-circle ${isSelected ? 'active' : ''}`}>
                    {index + 1}
                  </div>
                  <div>
                    <div className="route-card-time-distance">
                      <span className="route-time">{Math.round(route.duration)} phút</span>
                      <span className="route-distance-small">({route.distance.toFixed(2)} km)</span>
                    </div>
                    <div className="route-card-status">
                      {hasFlood ? (
                        <span className="status-badge-warning">
                          <AlertTriangle size={10} /> Cẩn thận
                        </span>
                      ) : (
              ...
              <div className={`route-card-content ${isSelected ? 'expanded' : ''}`}>
                  ...
                  <div className="ai-analysis-section">
                    {!hasAiAdvice ? (
                      ...
                      >
                        ...
                        {isAnalyzingRoute ? "Đang phân tích..." : "Hỏi Gemini về rủi ro"}
                      </button>
                    ) : (
