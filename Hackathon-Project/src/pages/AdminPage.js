...
const AdminPage = () => {
  ...
  // State quản lý danh sách thiết bị (2 con)
  const [devices, setDevices] = useState([
    { 
      id: "ESP32_01", 
      name: "Trạm Cầu Rồng (Node 1)", 
      latitude: 16.0611, 
      longitude: 108.2270, 
      wifiSSID: "WIFI_NODE_1",
      wifiPass: "12345678"
    },
    { 
      id: "ESP32_02", 
      name: "Trạm Ngũ Hành Sơn (Node 2)", 
      latitude: 16.0125, 
      longitude: 108.2442, 
      wifiSSID: "WIFI_NODE_2",
      wifiPass: "87654321"
    }
  ]);
  
  const [selectedDeviceId, setSelectedDeviceId] = useState("ESP32_01");
  
  // Lấy thiết bị đang được chọn để hiển thị ra form
  const currentDevice = devices.find(d => d.id === selectedDeviceId) || devices[0];

  // Hàm update config cho thiết bị đang chọn
  const handleConfigChange = (field, value) => {
    setDevices(prev => prev.map(d => 
      d.id === selectedDeviceId ? { ...d, [field]: value } : d
    ));
  };
    ...
    <div className="admin-container flex gap-6 overflow-hidden relative">
      ...
      <div className="w-64 flex-shrink-0 flex flex-col z-10">
        ...
        <div className="space-y-2 flex-1">
            ...
            <SidebarItem id="users" icon={<Users size={20} />} label="Người dùng" />
            <SidebarItem id="system" icon={<Activity size={20} />} label="Hệ thống" />
            <SidebarItem id="config" icon={<Sliders size={20} />} label="Cấu hình (Arduino)" />
        </div>
      ...
      <div className="flex-1 flex flex-col gap-6 z-10 h-[calc(100vh-40px)] overflow-y-auto pr-2 custom-scrollbar">
        ...
        {/* --- CONFIG TAB (Multi-Device Support) --- */}
        {activeTab === 'config' && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                {/* Device List Sidebar */}
                <div className="lg:col-span-1 flex flex-col gap-4">
                    <div className="admin-glass-panel p-4">
                        <h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2">
                            <Cpu size={20} className="text-indigo-600" /> Chọn thiết bị
                        </h3>
                        ...
                        <button className="mt-4 w-full py-2 border border-dashed border-slate-300 text-slate-500 rounded-xl hover:bg-slate-50 font-bold text-sm flex items-center justify-center gap-2">
                            + Thêm thiết bị mới
                        </button>
                    </div>

                    {/* Device Info Summary */}
                    <div className="admin-glass-panel p-4 bg-slate-800 text-white">
                        <h4 className="font-bold text-sm mb-3 text-slate-400 uppercase">Thông số hiện tại</h4>
                        <div className="space-y-2 font-mono text-xs">
                            <div className="flex justify-between">
                                <span className="text-slate-500">ID</span>
                                <span className="text-yellow-400">{currentDevice.id}</span>
                            </div>
                            <div className="flex justify-between">
                                <span className="text-slate-500">LAT</span>
                                <span className="text-green-400">{currentDevice.latitude}</span>
                            </div>
                            <div className="flex justify-between">
                                <span className="text-slate-500">LON</span>
                                <span className="text-green-400">{currentDevice.longitude}</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="lg:col-span-2">
                    <div className="admin-glass-panel p-8">
                        <div className="mb-6 border-b border-slate-200 pb-4 flex justify-between items-center">
                             <div>
                                <h3 className="text-xl font-bold text-slate-800 flex items-center gap-2">
                                    ...
                                    Cấu hình: {currentDevice.name}
                                </h3>
                                <p className="text-sm text-slate-500 mt-1">ID: {currentDevice.id}</p>
                             </div>
                        ...
                        <div className="space-y-6">
                             ...
                             <div>
                                <h4 className="text-sm font-bold text-indigo-600 uppercase mb-3">Thông tin cơ bản</h4>
                                <div className="grid grid-cols-1 gap-4">
                                    <div>
                                        ...
                                        <input 
                                            ...
                                            className="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none font-medium text-slate-700"
                                        />
                                    </div>
                                </div>
                             </div>
                             ...
                             {/* Section 2: Location */}
                             <div>
                                <h4 className="text-sm font-bold text-indigo-600 uppercase mb-3">Vị trí (GPS)</h4>
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-xs font-bold text-slate-500 mb-2">Vĩ độ (Latitude)</label>
                                        <input 
                                            type="number" 
                                            step="0.000001"
                                            value={currentDevice.latitude}
                                            onChange={(e) => handleConfigChange('latitude', e.target.value)}
                                            className="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none font-mono text-slate-700"
                                        />
                                    </div>
                                    <div>
                                        <label className="block text-xs font-bold text-slate-500 mb-2">Kinh độ (Longitude)</label>
                                        <input 
                                            type="number" 
                                            step="0.000001"
                                            value={currentDevice.longitude}
                                            onChange={(e) => handleConfigChange('longitude', e.target.value)}
                                            className="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none font-mono text-slate-700"
                                        />
                                    </div>
                                </div>
                                <button className="mt-2 text-xs text-blue-600 font-bold hover:underline flex items-center gap-1">
                                    <Map size={12} /> Chọn trên bản đồ
                                </button>
                             </div>

                             {/* Section 3: Network */}
                             <div>
                                <h4 className="text-sm font-bold text-indigo-600 uppercase mb-3">Kết nối mạng (WiFi)</h4>
                                <div className="grid grid-cols-2 gap-4 bg-slate-50 p-4 rounded-xl border border-slate-200">
                                    <div>
                                        <label className="block text-xs font-bold text-slate-500 mb-2">WiFi SSID</label>
                                        <input 
                                            type="text" 
                                    ...
                                    <div>
                                        <label className="block text-xs font-bold text-slate-500 mb-2">WiFi Password</label>
                                        <input 
                                            type="password" 
                                            value={currentDevice.wifiPass}
                             ...
                             <div className="flex gap-3 pt-4 border-t border-slate-200">
                                <button 
                                    onClick={() => {
                                        // Logic gửi lệnh riêng cho thiết bị đang chọn
                                        const timestamp = new Date().toLocaleTimeString();
                                        const cmd = `CMD_SEND [${currentDevice.id}]: SAVE_CONFIG name="${currentDevice.name}" lat=${currentDevice.latitude} lon=${currentDevice.longitude} ssid="${currentDevice.wifiSSID}"`;
                                        
                                        setLogs(prev => [...prev, `[${timestamp}] ${cmd}`]);
                                        
                                        // Gửi lệnh thật lên Firebase (vào nhánh commands của device đó)
                                        push(ref(db, `commands/${currentDevice.id}`), {
                                            command: "UPDATE_CONFIG",
                                            params: currentDevice,
                                            timestamp: Date.now()
                                        });

                                        alert(`Đã gửi cấu hình xuống thiết bị ${currentDevice.id}!`);
                                    }}
                                    className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 rounded-xl shadow-lg shadow-indigo-500/30 transition-all flex items-center justify-center gap-2"
                                >
                                 ...
                                 <button 
                                    onClick={() => {
                                        ...
                                        alert(`Đã gửi lệnh khởi động lại ${currentDevice.id}!`);
                                    }}
                                    className="px-6 bg-slate-200 hover:bg-slate-300 text-slate-700 font-bold py-3 rounded-xl transition-all"
                                >
                                    Restart
                                </button>
                             </div>
